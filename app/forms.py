...
from wtforms import (
    ...
    BooleanField, SelectField, PasswordField, EmailField, HiddenField,
    IntegerRangeField,
)
...
class ProductForm(FlaskForm):
    ...
    old_price = DecimalField(
        "Old Price (₦) – optional", validators=[Optional(), NumberRange(min=0)]
    )
    stock_quantity = IntegerField(
        "Stock Quantity", validators=[DataRequired(), NumberRange(min=0)], default=0
    )
...
# ─────────────────────────────────────────────────────────────────────────────
# Product search (GET form – no CSRF needed)
# ─────────────────────────────────────────────────────────────────────────────
class SearchForm(FlaskForm):
    class Meta:
        csrf = False  # GET request, no CSRF token needed
