        ...
        for idx, f in enumerate(files[:5]):
            if f and f.filename:
                ...
                db.session.add(ProductImage(
                    ...
                    is_main=is_main, sort_order=idx))
                if is_main:
                    product.image = url
                saved += 1

        db.session.commit()
        flash(f"✅ Product '{product.name}' added with {saved} image(s)!", "success")
...
def edit_product(pid):
    ...
    if form.validate_on_submit():
        ...
        for idx, f in enumerate(files[:slots]):
            if f and f.filename:
                url = save_product_image(f)
                is_main = (existing == 0 and idx == 0)
                db.session.add(ProductImage(
                    product_id=product.id, image_url=url,
                    is_main=is_main, sort_order=existing + idx))
                if is_main:
                    product.image = url
...
@admin_bp.route("/products/<int:pid>/image/<int:iid>/delete", methods=["POST"])
@login_required
def delete_image(pid, iid):
    ...
    if was_main:
        ...
        if next_img:
            ...
            product.image = next_img.image_url
        else:
...
@admin_bp.route("/products/<int:pid>/image/<int:iid>/set-main", methods=["POST"])
@login_required
def set_main_image(pid, iid):
    product = Product.query.get_or_404(pid)
    for img in product.images.all():
        img.is_main = False
    main = ProductImage.query.get_or_404(iid)
    main.is_main  = True
    product.image = main.image_url
    db.session.commit()
...
def toggle_featured(pid):
    ...
    db.session.commit()
    flash(f"{'Featured' if product.featured else 'Unfeatured'}.", "info")
    return redirect(url_for("admin.products"))
...
def add_category():
    ...
    if form.validate_on_submit():
        ...
        if Category.query.filter_by(slug=slug).first():
            ...
            db.session.add(Category(name=form.name.data, slug=slug,
...
def add_testimonial():
    ...
    if form.validate_on_submit():
        db.session.add(Testimonial(customer_name=form.customer_name.data,
                                   location=form.location.data or "Ilorin, Kwara State",
                                   message=form.message.data, rating=form.rating.data,
...
def edit_testimonial(tid):
    ...
    if form.validate_on_submit():
        ...
        t.message = form.message.data; t.rating = form.rating.data
...
def toggle_testimonial(tid):
    t = Testimonial.query.get_or_404(tid)
    t.is_approved = not t.is_approved; db.session.commit()
    flash(f"{'Approved' if t.is_approved else 'Hidden'}.", "info")
    return redirect(url_for("admin.testimonials"))